...
namespace truthful_terrapins_heist_ii
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (bank.IsSecure)
            {
                Console.WriteLine("The heist was not succesfull Kaci goes to prison.");
            }
            else
            {
                Console.WriteLine($"The heist was succesfull Kaci goes to prison. The total Score was ${bank.CashOnHand}");
                foreach (IRobber robber in crew)
                {
                    Console.WriteLine($"{robber.Name} recieves ${bank.CashOnHand * ((float)robber.PercentageCut / 100)}");
                }
